-- Create caregivers table for normalized names and color assignments
...
-- Add caregiver_id to check_in_outs table
...
BEGIN
  ...
  ELSE
    ...
    IF new_color IS NULL THEN
      new_color := available_colors[1 + (array_length(used_colors, 1) % array_length(available_colors, 1))];
    END IF;
  END IF;
...
BEGIN
  FOR rec IN SELECT DISTINCT caregiver_name FROM check_in_outs WHERE caregiver_name IS NOT NULL
  LOOP
    -- Get or create caregiver
    new_caregiver_id := get_or_create_caregiver(rec.caregiver_name);

    -- Update check_in_outs records
    UPDATE check_in_outs
    SET caregiver_id = new_caregiver_id
    WHERE caregiver_name = rec.caregiver_name AND caregiver_id IS NULL;
  END LOOP;
END $$;
